...
static __FORCE_INLINE__ return_t physIslandPresolveConstraints(physIsland *const __RESTRICT__ island, const float dt_s){
	...
	joint = island->joints;
	while(joint != NULL){
		physJointPresolveConstraints(joint, dt_s);
...
static __FORCE_INLINE__ void physIslandSolveConstraints(const physIsland *const __RESTRICT__ island, const float dt_s){
...
return_t physIslandTick(physIsland *const __RESTRICT__ island, const float dt_s){
	...
	if(physIslandPresolveConstraints(island, dt_s) < 0){
		return -1;
	}
	...
	physIslandSolveConstraints(island, dt_s);
